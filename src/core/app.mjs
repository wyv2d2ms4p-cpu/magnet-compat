 ...
 * ②確認画面。
 *
 * **出典行（`.evidence`）は操作ボタンより後ろに置く。** 出典は「表示された仕様を
 * 疑ったときに読むもの」で、交換作業中の動線には要らない。仕様欄と外形図の間に
 * 置いていたときは、`srcNote` を持つレコードでこの1行が画面1枚分を占めていた
 * （MS3749-A-O25 で813字。うち現場が見る状態表示は「型式 確認済 / 寸法 確認済 /
 * 仕様 確認済」の18字だけで、残り795字は出典の本文）。ボタンの手前にあると、
 * 「この仕様で互換品を表示」へ到達するまでにその本文をスクロールで越えることになる。
 *
 * 位置を変えただけで、出典の内容も文言も削っていない（出典は削らない約束）。
 * 行そのものは DOM に残るので、`.evidence .badge` の折り返しは今までどおり効く。
 */
...
function viewResult() {
   ...
   *
   * 既定文に書けるのは**どのカテゴリでも真であること**だけ。ここには以前
   * 「互換候補が登録されていません。電気的に成立しない組み合わせ（出力極性・配線本数の
   * 相違）は候補から除外しています」が置かれていたが、この2条件は `sensorGate` にしか
   * 無く、インバータ・接触器・電磁開閉器・サーマルの gate は参照していない。
   * 存在しない除外理由を9カテゴリ共通の既定文として名乗っていたことになる
   * （文面はセンサー4カテゴリの `emptyNote` へ移した）。
   *
   * 同じ理由で「登録されていません」とも断定しない。0件は登録の有無とは限らず、
   * 同クラス・同相の型式が登録済みで判定の窓を外れているだけのこともある
   * （インバータの0件20件がこれ）。書けるのは「このアプリは候補を出せなかった」まで。
   */
  const empty = cat.emptyNote(m) ||
    `<b class="amber">互換品候補は見つかりませんでした。</b>
     <div>このカテゴリの判定条件を満たす型式が、登録データの中にありませんでした。
     判定に使えるのは登録された値だけなので、置換えできる型式が無いことを意味するものでは
     ありません。メーカーの選定資料で確認してください。</div>`;
  /**
