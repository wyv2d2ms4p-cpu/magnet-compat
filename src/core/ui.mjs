 ...
 * 交換前の機器の定格であって、実際の負荷電流ではない。34A の機器が付いていても
 * 実負荷が 20A なら 26A 品で足りるので、「容量不足」「使用不可」と書くと、
 * 成立する置換えを現場が捨てることになる。
 *
 * 表示が無い候補について後半で断りを入れているのは、無印が「基準以上」と
 * 「比較できる定格が登録されていない」の2つを兼ねるため。無印を黙って
 * 「基準以上」と読ませない。
 */
export function loadCheckNote(n) {
  if (!n) return '';
  return `<div class="load-note">「基準より小さい」は、交換前の機器の定格との比較です。
    実際の負荷電流は定格とはかぎらないので、現場で確認してください。
    表示が無い候補は、基準以上か、比較できる定格が登録されていないかのどちらかです。</div>`;
}
