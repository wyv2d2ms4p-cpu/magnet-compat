 ...
 *
 * この文面は元コアの既定文（`src/core/app.mjs`）だった。しかし名指ししている2条件
 ...
 *
 * `emptyNote(m)` は基準機しか受け取らないので、**アプリはその画面で何が落ちたかを
 * 実行時に知らない**。移設直後の文面は「（出力極性・配線本数の相違）は候補から
 * 除外しています」と、知らないことを画面ごとの事実として語っていた。
 * 実測すると（`docs/sensor-empty-note-audit.md`）、候補0件16件のうち
 *   ・GX-4M は候補を失った理由がねじ径で、名指しの2条件は結果を1件も動かしていない
 *   ・5件は互換系列を共有する相手が0件で、2条件は何も説明していない
 * だった。「候補に出すのは〜だけ」という必要条件の言い方にすると、その画面でどれが
 * 効いたかを主張しないので16件すべてで真になる。最後の1文で、画面ごとの理由を
 * 出せないことを明示する（段4で「容量不足」と書かないのと同じ原則）。
 *
 * 見出しで「登録されていません」と断定しないのは、0件が登録の欠落とは限らないため。
 * 16件中11件は互換系列を共有する型式が登録済みで、判定で外れただけ。この語は
 * 検索画面が「実在確認済みのデータが未登録です」の意味で使っている（`core/app.mjs`）。
 *
 * 後継品の経路（`a.id === m.successorId`）を書き落とさないこと。互換系列を共有しない
 * まま後継指定だけで通るペアが実在する（I-215.95HTF → PR-219.A3HTF）。
 * gate を変えたらこの文面も追随させること。名指ししている条件が `sensorGate` の
 * 必要条件のままであることは `tools/test-sensor-empty.mjs` が検算する。
 */
export function sensorEmptyNote() {
  return `<b class="amber">互換品候補は見つかりませんでした。</b>
     <div>このカテゴリが候補に出すのは、メーカー指定の後継品か、同じ互換系列として登録された
