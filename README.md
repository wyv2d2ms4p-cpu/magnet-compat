...
- **型式の正規化は「集める」と「見分ける」で分ける。**
  `normLoose`（空白・ハイフン・小数点を除去）は候補を集めるためだけに使い、
  ...
  **英字 `O` と数字 `0` の同一視も「集める」側だけに入れる**（`O` → `0` に寄せる）。
  銘板の `MS3749-A-O25` をゼロで打つと0件になる問題への対応で、これは読み取りの
  曖昧さであって値そのものの違いではない。`I`/`1`・`B`/`8` は現場の取り違え事例が
  無いため対象外。
