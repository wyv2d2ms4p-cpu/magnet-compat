 ...
 *
 * この検査が見るのは `data/<id>.json` と `src/categories/<id>.mjs` の対応で、
 * どちらも人手で置く（カテゴリ追加にはこの2つのほかに `ADDED_SPEC_KEYS` への
 * `specs` キーの宣言も要る。README「カテゴリを追加する」）。
 * 片方だけ入った状態は静かに壊れる——データだけならチップに
