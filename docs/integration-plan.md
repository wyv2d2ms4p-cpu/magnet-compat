...
フェーズ2・3の実作業は、カテゴリごとにサブエージェント `compat-researcher`
（`.claude/agents/compat-researcher.md`）を並列で走らせて進める。
各エージェントは担当カテゴリの `data/<category>.json` だけを編集し、
`npm run verify` の「追加レコードの規約」が出典の無いレコードを機械的に弾く。
...
## データ拡充の受け入れ条件（フェーズ2以降）

移行の非破壊性を検証する仕組み（`tools/verify-data.mjs`）は、移行元HTMLと
`data/**` を突合する。ここに**あとから追加したレコード**が入ると、移行元に対応が
無いため件数もID集合も一致しなくなる。かといって検証を緩めると、移行が壊れても
気づけない。両立させるため、レコードを2種類に分けて別々の規則で見る。

| 種別 | 判定 | 検査内容 |
|---|---|---|
| 移行由来 | IDが移行元に存在する | 件数・ID集合・値の往復一致（従来どおり厳格） |
| 追加分 | IDが移行元に無い | 下記の受け入れ条件 |

### 追加レコードの受け入れ条件

- `modelStatus` は `catalog-confirmed`（`provisional` は `_seed/` にしか置かない）
- `evidence` は3側面そろい、`model.state` は `verified`
- **`evidence.model` に `srcUrl` か `srcNote` が必須。**
  「出典が確認できない型式は追加しない」を人手のレビューに委ねず機械的に強制する
- `srcUrl` があれば `http(s)://` であること
- トップレベルのキーは `RECORD_KEYS` の範囲内（`sucessorId` のような綴り間違いは
  黙って無視されるだけなので落とす）
- `specs` のキーは `SPEC_MAP[カテゴリ]` の値域内
- `category` はファイル名と一致

### 追加分を消さないための安全弁

`tools/extract.mjs` は移行元HTMLから `data/**` を作り直す（冪等）。
追加レコードは移行元に存在しないので、素直に走らせると**調査して足したぶんが
黙って消える**。既存ファイルにしか無いIDを見つけたら停止し、
本当に作り直すときだけ `--force` を要求する。
